...
printf '\n[worker] Connecting to Ray head at %s:%s with %s CPUs reserved for the worker...\n' "$HEAD_IP" "$RAY_PORT" "$WORKER_CPUS"
...
docker run --gpus all \
  ...
  -w /workspace \
  -it "$IMAGE_NAME" \
  bash -lc "ray start --address='${HEAD_IP}:${RAY_PORT}' --num-cpus=${WORKER_CPUS} --block"
